...
const MobileFeeManagementHub: React.FC = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'assign':
        return permissions.canModifyFeeStructure ? <EnhancedFeeAssignment /> : <div className="text-center py-8 text-gray-500">
      ...
      case 'bulk':
        return permissions.canModifyFeeStructure ? <BulkFeeActions selectedRecords={selectedRecords} onBulkUpdate={handleBulkUpdate} onClear={handleClearSelection} /> : <div className="text-center py-8 text-gray-500">
            <Lock className="w-12 h-12 mx-auto mb-4 opacity-50" />
    ...
    return <div className="container mx-auto py-6 px-4">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
          ...
          gridTemplateColumns: `repeat(${tabs.length}, 1fr)`
            ...
            {tabs.map(tab => <TabsTrigger key={tab.id} value={tab.id} className="data-[state=active]:bg-blue-500 data-[state=active]:text-white flex items-center gap-2">
  ...
  return <div className="flex flex-col h-full bg-gray-50">
            ...
            <SheetContent side="bottom" className="max-h-[50vh]">
              <div className="py-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  return <Button key={tab.id} variant={isActive ? "default" : "outline"} onClick={() => handleTabChange(tab.id)} className={`h-16 flex flex-col items-center justify-center gap-2 ${isActive ? user?.role === 'chairman' ? 'bg-gradient-to-r from-purple-600 to-blue-600' : 'bg-blue-600' : 'hover:bg-gray-50'}`}>
      ...
      {user?.role === 'chairman' && <div className="sticky bottom-0 bg-gradient-to-r from-purple-600 to-blue-600 text-white px-4 py-2 text-center">
          <p className="text-xs font-medium">Chairman's View </p>
        </div>}
    </div>;
};
