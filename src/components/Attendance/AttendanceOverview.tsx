...
const AttendanceOverview: React.FC = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'overview':
        return (
          <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-bold text-green-600`}>{presentToday}</p>
                  </div>
                  <div className="p-2 bg-green-100 rounded-lg">
                    <Users className={`${isMobile ? 'w-5 h-5' : 'w-6 h-6'} text-green-600`} />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-bold text-red-600`}>{absentToday}</p>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-bold text-yellow-600`}>{leaveToday}</p>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-bold text-blue-600`}>{attendanceRate}%</p>
                  </div>
            ...
            {/* Today's Attendance Table - Hidden on mobile, replaced with cards */}
            {!isMobile && (
      ...
      case 'marking':
        return isMobile ? <MobileAttendanceMarking /> : <AttendanceMarking />;
      case 'analytics':
  ...
  return (
    <div className="space-y-6">
      <div className={`${isMobile ? 'flex-col space-y-4' : 'flex'} justify-between items-start ${!isMobile && 'sm:items-center'} gap-4`}>
        <h2 className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold text-gray-800`}>Attendance Management</h2>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" className="flex items-center space-x-2" size={isMobile ? 'sm' : 'default'}>
