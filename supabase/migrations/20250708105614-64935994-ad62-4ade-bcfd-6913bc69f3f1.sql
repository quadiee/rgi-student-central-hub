-- Add RLS policies for chairman role to access department analytics

-- Enable RLS on department_fee_analytics view
ALTER TABLE public.department_fee_analytics ENABLE ROW LEVEL SECURITY;

-- Create policy for chairman to view all department analytics
CREATE POLICY "Chairman can view all department analytics"
...
USING (
  EXISTS (
    ...
    AND profiles.role = 'chairman'::user_role
  )
);
